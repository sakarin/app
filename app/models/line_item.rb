class LineItem < ActiveRecord::Base
  ...
  scope :by_inventory, lambda {|shipment_id|
    joins(" INNER JOIN inventory_units ON line_items.order_id = inventory_units.order_id AND line_items.variant_id = inventory_units.variant_id").
    where(" inventory_units.state LIKE 'sold' AND inventory_units.shipment_id = ? ", shipment_id)
  }
  ...
  private
    def update_inventory
      ...
      if self.new_record?
        InventoryUnit.increase(self.order, self.variant, self.quantity, self)
      elsif old_quantity = self.changed_attributes["quantity"]
        if old_quantity < self.quantity
          InventoryUnit.increase(self.order, self.variant, (self.quantity - old_quantity), self)
